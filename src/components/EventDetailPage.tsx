...
export function EventDetailPage({ eventId = '1', onNavigate }: EventDetailPageProps) {
  ...
  const [isJoined, setIsJoined] = useState(event.currentParticipants.includes(user?.id || ''));
  ...
  const handleLeaveEvent = () => {
    ...
    if (isJoined) {
      event.currentParticipants = event.currentParticipants.filter(id => id !== user.id);
      setIsJoined(false);
